...
exports.getDriveThruRestaurantsOnTheWay = function (req, res) {
    ...
    //Create set of points in formatting required for simplification
    var points = [];
    decodedPolyLine.forEach(function(elem,index){
        points[points.length] = {x: elem[0], y:elem[1]};
    });

    //Simplify polyline to make less DB lookups
    var simplifiedPolyline = simplify.simplify(points, 0.006);


    //Simple lock to synchronize async calls before response
